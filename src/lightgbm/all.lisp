...
;;;; wrapper, every %-function), `cl-gbdt/src/lightgbm/classes' (Layer 1: the backend's CLOS
;;;; types and the `initialize-backend'/`shutdown-backend' pair that opens and closes the
;;;; shared library) and `cl-gbdt/src/lightgbm/protocol' (Layer 2: the thirteen protocol
;;;; methods) into one package. This is what `cl-gbdt/lightgbm' (see cl-gbdt.asd) depends
;;;; on, following the same shape `src/xgboost/all.lisp' uses for the same reason.
;;;;
