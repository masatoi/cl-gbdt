...
- **Layer 1でのdataset / booster構築 — 完了。残るはmodel永続化とmetadata照会** — 他の三件と違い、これは新機能ではなくLayer 1 / Layer 2分離が残した負債であった。したがって実利用要求を待たず、層分離の次段の最初の項目として着手し、閉じた。両backendとも `src/<backend>/api.lisp` に六つの操作 — `create-dataset`、`create-booster`、`update-one-iteration`、`predict`、`free-dataset`、`free-booster` — を置いたので、`cl-gbdt/lightgbm` あるいは `cl-gbdt/xgboost` だけを読み込んだcallerが、datasetを構築し、その上にboosterを作り、一反復ずつ進め、予測し、両方を解放できる。unified APIがimageに一切存在しない状態でそれが成り立つことは、`tests/functional/lightgbm-standalone.lisp` と `tests/functional/xgboost-standalone.lisp` が示す。両fileの依存はそのbackendのpublic package一つだけであり、`tools/ci/check-leaf-systems.lisp` が個別の新規processで各systemを単独loadするため、この主張は文章ではなくbuildが担保する。§3の「Layer 2の各methodは、可能な限りLayer 1のbackend-specific safe APIへ委譲する」という要求も、13 methodのうち五つ — `make-dataset`、`predict`、`update-one-iteration`、`free-dataset`、`free-booster` — が手続き全体をこれらへ委譲する形で果たされた。`train` だけが例外で、Layer 1に対応する `create-booster` がありながら自前でboosterを構築する。`booster-best-iteration` は構築時にしか書けないslotであり(src/handle.lisp)、その値はloopが終わるまで定まらないためである。各 `protocol.lisp` の `train` の生成箇所に、測定にもとづく理由が記してある。次の一段が引き取るのは、Layer 1に対応物がない残りの操作である。`save-model`、`load-model`、`model-to-string`、`feature-importance`、`evaluation`、`dataset-num-rows`、`dataset-num-features` の七つは、いずれもunified APIのmethodとしてしか存在しない。一方、training report、early stopping、`train` の `:objective` と `:evaluation` はLayer 2固有の概念であって対応するLayer 1操作を持たないので、この一覧には入らない。
