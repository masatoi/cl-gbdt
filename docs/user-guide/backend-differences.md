...
| An entry a `csr-matrix` does not store | Read as `0.0` while `zero_as_missing` is off, which is the default -- that flag *can* change it. See [LightGBM's `zero_as_missing`, measured](data-and-prediction.md#lightgbms-zero_as_missing-measured) | Read as *missing*, and no key this project has found changes that -- measured as none found, not established that none exists. So dropping zeros in a conversion silently changes results rather than erroring. `make-csr-matrix`'s `:implicit-value` lets a caller declare which reading their data means and get a refusal rather than different numbers. See [An absent entry is not a zero](data-and-prediction.md#an-absent-entry-is-not-a-zero-and-the-two-libraries-disagree-about-it) |
| `make-csr-matrix`'s `:implicit-value` | Accepts a zero -- its own reading of absence in its default configuration -- and refuses `:missing` | The reverse: accepts `:missing`, refuses a zero. `NIL` and `:none` pass on both. A program that declares one of the two values therefore moves between backends only by changing the declaration, which is the refusal doing its job. See [An absent entry is not a zero](data-and-prediction.md#an-absent-entry-is-not-a-zero-and-the-two-libraries-disagree-about-it) |
| `make-dataset`'s and `predict`'s `:missing` | `capability-unavailable` for any non-`NIL` value, a `NaN` included -- LightGBM's C API has no missing-value key at all | Supported: the sentinel is a key its creation and prediction config JSONs already read, compared against the data at single precision. See [Missing values](data-and-prediction.md#missing-values) |
...
*verified* one (the versions the functional suite has actually run against)
