...
| An entry a `csr-matrix` does not store | Read as `0.0` -- `zero_as_missing` is off by default | Read as *missing*. No config key changes either; that is what CSR means to each library, so dropping zeros in a conversion silently changes results rather than erroring. `make-csr-matrix`'s `:implicit-value` lets a caller declare which reading their data means and get a refusal rather than different numbers. See [An absent entry is not a zero](data-and-prediction.md#an-absent-entry-is-not-a-zero-and-the-two-libraries-disagree-about-it) and [LightGBM's `zero_as_missing`, measured](data-and-prediction.md#lightgbms-zero_as_missing-measured) |
