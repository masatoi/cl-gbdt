...
| An entry a `csr-matrix` does not store | Read as `0.0` -- `zero_as_missing` is off by default | Read as *missing*. No config key changes either; that is what CSR means to each library, so dropping zeros in a conversion silently changes results rather than erroring. See [An absent entry is not a zero](data-and-prediction.md#an-absent-entry-is-not-a-zero-and-the-two-libraries-disagree-about-it) |
