...
| `XGDMatrixSaveBinary` | Write an already-constructed DMatrix to XGBoost's own binary format, so a second run over the same data pays for construction once rather than every time. Pairs with `XGDMatrixCreateFromURI`, now wrapped as `create-dataset-from-file`; on its own it is write-only. Same row as `LGBM_DatasetSaveBinary`. The header excludes `QuantileDMatrix` and external-memory DMatrix, neither of which this project builds, so that restriction costs nothing today. |
